      ...
      <div v-if="!downloadLink">
        <form @submit.prevent="handleSubmit" id="conversion-form">
          <div class="form-group">
            <label for="source_file">1. 选择文件</label>
            <input
              type="file"
              id="source_file"
              ref="fileInput"
...
export default {
  ...
  mounted() {
    ...
    const saved = sessionStorage.getItem(`pdfConvertToPdf_${token}`)
    if (saved) {
      try {
        ...
        if (Date.now() < expiry) {
          ...
          sessionStorage.removeItem(`pdfConvertToPdf_${token}`)
        }
      } catch { sessionStorage.removeItem(`pdfConvertToPdf_${token}`) }
    }
  ...
  methods: {
    async loadPageData() {
      ...
      try {
        ...
        const response = await pdfApi.getConvertToPdf(params)
        if (response && response.success) {
          this.maxFileSize = response.data.max_file_size
          this.supportedExtensions = response.data.supportedExtensions || []
        }
        this.pageLoading = false
      } catch (error) {
    ...
    handleFileChange(event) {
      ...
      if (file) {
        ...
        if (!this.supportedExtensions.includes(fileExtension)) {
          this.errorMessage = `不支持的文件格式 (.${fileExtension})。支持的格式：${this.supportedExtensions.join(', ')}`
    ...
    async handleSubmit() {
      ...
      try {
        ...
        const convertResponse = await pdfApi.commitConvertToPdf(
          {
            source_file: signData.dir + this.selectedFile.name
          },
          { t: token }
        )
        ...
        if (convertResponse && convertResponse.success) {
          ...
          sessionStorage.setItem(`pdfConvertToPdf_${token}`, JSON.stringify({ link: this.downloadLink, expiry: Date.now() + 5 * 60 * 1000 }))
        } else {
